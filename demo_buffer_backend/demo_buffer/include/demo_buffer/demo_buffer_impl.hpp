...
#ifndef DEMO_BUFFER__DEMO_BUFFER_IMPL_HPP_
#define DEMO_BUFFER__DEMO_BUFFER_IMPL_HPP_
...
public:
  ...
  std::shared_ptr<void> create_descriptor(const rmw_gid_t & subscriber_gid) const override
  {
    ...
    // Copy data to descriptor as a std::vector, then assign to Buffer
    // This ensures the descriptor's data field uses CPU backend and avoids
    // recursive endpoint-aware serialization that can cause double-free issues
    std::vector<uint8_t> cpu_data(storage_.size() * sizeof(T));
    if (!storage_.empty()) {
      std::memcpy(
        cpu_data.data(),
        storage_.data(),
    ...
    // Assign vector to Buffer - this creates a CPU-backed buffer
    descriptor->data = std::move(cpu_data);
...
#endif  // DEMO_BUFFER__DEMO_BUFFER_IMPL_HPP_
