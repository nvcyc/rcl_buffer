...
def resolve_exe(ws: Path, lang: str, role: str) -> List[str]:
    ...
    For C++ binaries:  ['/path/to/perf_publisher']
    For Python scripts: [sys.executable, '/path/to/perf_publisher_py.py']
    """
        ...
        if r.sub_results:
            ...
            for s in r.sub_results:
                lines.append(
                    ...
                    f"lat={s.latency_min_us:.0f}/{s.latency_mean_us:.0f}/"
                    f"{s.latency_max_us:.0f} us (min/mean/max)"
                )
    return "\n".join(lines)
