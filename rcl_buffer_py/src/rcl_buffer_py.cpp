...
/// Minimal Python wrapper around rcl_buffer::Buffer<uint8_t>.
///
...
{
public:
  explicit PyBuffer(std::shared_ptr<rcl_buffer::Buffer<uint8_t>> buf)
  ...
  rcl_buffer::Buffer<uint8_t> * get_raw_buffer() {return buffer_.get();}

  std::string repr() const
  {
    return "rcl_buffer.Buffer(size=" + std::to_string(buffer_->size()) +
           ", backend='" + buffer_->get_backend_type() + "')";
  }

private:
  std::shared_ptr<rcl_buffer::Buffer<uint8_t>> buffer_;
};

PYBIND11_MODULE(_rcl_buffer_py, m)
{
  m.doc() = "Python bindings for rcl_buffer::Buffer<uint8_t>";
  ...
  m.def("is_buffer", [](py::object obj) -> bool {
    ...
    }, "Check if the given object is an rcl_buffer.Buffer");
  ...
  m.def("_take_buffer_from_ptr", [](uintptr_t ptr) -> PyBuffer {
      auto * buf = reinterpret_cast<rcl_buffer::Buffer<uint8_t> *>(ptr);
      auto shared_buf = std::shared_ptr<rcl_buffer::Buffer<uint8_t>>(buf);
