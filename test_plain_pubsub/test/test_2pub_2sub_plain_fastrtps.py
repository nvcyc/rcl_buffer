...
# Uses plain std_msgs/String — no buffer backends involved.
...
def generate_test_description():
    """Generate launch description for 2 pub / 2 sub per topic, plain String over FastRTPS."""
    publisher_1 = Node(
        package='test_plain_pubsub',
        executable='plain_string_publisher_node',
        name='plain_publisher_1',
        output='screen',
        parameters=[{
            'topic_name': 'topic1_string',
            'publish_rate_ms': 200,
    ...
    subscriber_1a = Node(
        package='test_plain_pubsub',
        executable='plain_string_subscriber_node',
        name='plain_subscriber_1a',
        output='screen',
        parameters=[{
            'topic_name': 'topic1_string',
            'count_topic_prefix': 'topic1',
    ...
    publisher_2 = Node(
        package='test_plain_pubsub',
        executable='plain_string_publisher_node',
        name='plain_publisher_2',
        output='screen',
        parameters=[{
            'topic_name': 'topic2_string',
            'publish_rate_ms': 200,
...
class TestPlain2Pub2SubFastRTPS(unittest.TestCase):
    """Test case for 2 pub / 2 sub per topic, plain String over FastRTPS."""
    ...
    def setUp(self):
        self.node = rclpy.create_node('test_plain_2pub_2sub_fastrtps')
    ...
    def test_plain_2pub_2sub_messages_delivered(self):
        """Test 2 plain String publishers to 2 subscribers each over FastRTPS."""
...
class TestPlain2Pub2SubFastRTPSShutdown(unittest.TestCase):
    """Test shutdown behavior for plain 2 pub / 2 sub over FastRTPS."""
