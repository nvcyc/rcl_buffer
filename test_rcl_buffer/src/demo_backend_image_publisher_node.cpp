...
private:
  void timer_callback()
  {
    ...
    if (backend_mode_ == "demo") {
      ...
      // NOTE: We link demo_buffer (the impl-only package), NOT demo_buffer_backend
      // (the pluginlib plugin). Directly linking a pluginlib plugin library causes
      // a double-free on shutdown due to class_loader's ProxyExec static being
      // destroyed twice.
