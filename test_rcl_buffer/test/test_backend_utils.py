...
class BackendTestMonitor:
    ...
    def __init__(self, node: RclpyNode, subscriber_suffixes: Optional[List[str]] = None):
        ...
        ----------
        node : RclpyNode
            ROS2 node to use for subscriptions.
        subscriber_suffixes : Optional[List[str]], optional
            List of suffixes for subscriber count/validation topics.
    ...
    def spin_until_messages(self, target_count: int, timeout_sec: float = 15.0,
        ...
        ----------
        target_count : int
            Target number of messages to receive.
        timeout_sec : float, optional
            Timeout in seconds (default: 15.0).
        check_all_subscribers : bool, optional
            If True, checks that ALL subscribers reach target.
            If False, checks that ANY subscriber reaches target (default: True).

        Returns
        -------
        bool
            True if target was reached, False if timeout occurred.
