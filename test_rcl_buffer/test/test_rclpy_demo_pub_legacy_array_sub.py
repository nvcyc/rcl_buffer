...
#
# Launch test: rclpy demo publisher to rclpy legacy array.array subscriber.
#
...
# memoryview) and has NO imports from rcl_buffer.
#
# This validates that existing rclpy user code does not break when
# the received uint8[] field is backed by an rcl_buffer.Buffer.
...
class TestRclpyDemoPubLegacyArraySub(unittest.TestCase):
    ...
    def setUp(self):
        self.node = rclpy.create_node('test_demo_pub_legacy_sub')
...
class TestRclpyDemoPubLegacyArraySubShutdown(unittest.TestCase):
