...
def generate_test_description():
    service_node = Node(
        ...
        output='screen',
        parameters=[{
            'service_name': 'test_get_parameter_types',
            'backend_mode': 'cpu',
        }],
    )
    client_node = Node(
...
class TestRclpyServiceCpuFastRTPS(unittest.TestCase):
    """Validate rclpy CPU service request/response over FastRTPS."""
    ...
    def setUp(self):
        self.node = rclpy.create_node('test_rclpy_service_cpu_fastrtps')
    ...
    def test_service_cpu_path(self):
        start = time.time()
        while (
...
class TestRclpyServiceCpuFastRTPSShutdown(unittest.TestCase):
    """Validate clean shutdown for CPU service test."""
