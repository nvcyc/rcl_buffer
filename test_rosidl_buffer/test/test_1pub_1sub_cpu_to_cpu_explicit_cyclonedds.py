...
#
# Launch test: 1 publisher to 1 subscriber, CPU backend to CPU backend (CycloneDDS)
...
def generate_test_description():
    """Generate launch description for CPU-to-CPU test over CycloneDDS."""
    publisher_node = Node(
...
class TestCpuToCpuExplicitCycloneDDS(unittest.TestCase):
    """Test case for CPU-to-CPU pub/sub over CycloneDDS with explicit cpu option."""
    ...
    def setUp(self):
        self.node = rclpy.create_node('test_cpu_to_cpu_explicit_cyclonedds')
    ...
    def test_cpu_to_cpu_explicit_messages_delivered(self):
        """Test CPU pub to CPU sub over CycloneDDS with explicit acceptable_buffer_backends."""
...
class TestCpuToCpuExplicitCycloneDDSShutdown(unittest.TestCase):
    """Test shutdown for CPU to CPU with explicit cpu option over CycloneDDS."""
