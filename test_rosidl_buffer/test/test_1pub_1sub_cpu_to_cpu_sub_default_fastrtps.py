...
#
# Launch test: cpu pub to cpu sub without setting acceptable_buffer_backends (FastRTPS)
# Verifies that the default subscription option (not set) results in CPU-only.
...
def generate_test_description():
    """CPU pub to sub without setting acceptable_buffer_backends (uses default)."""
    publisher_node = Node(
...
class TestCpuCpuSubDefaultFastRTPS(unittest.TestCase):
    """CPU to CPU with default acceptable_buffer_backends (not set: CPU-only)."""
    ...
    def setUp(self):
        self.node = rclpy.create_node('test_cpu_cpu_sub_default_fastrtps')
