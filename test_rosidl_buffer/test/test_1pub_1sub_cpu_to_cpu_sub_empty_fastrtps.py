...
#
# Launch test: cpu pub to cpu sub with acceptable_buffer_backends="" (empty string) (FastRTPS)
# Verifies that empty string is treated as CPU-only (backward compat).
...
def generate_test_description():
    """CPU pub to sub with acceptable_buffer_backends='' (empty)."""
    publisher_node = Node(
...
class TestCpuCpuSubEmptyFastRTPS(unittest.TestCase):
    """CPU to CPU with acceptable_buffer_backends='' (empty string: CPU-only)."""
    ...
    def setUp(self):
        self.node = rclpy.create_node('test_cpu_cpu_sub_empty_fastrtps')
    ...
    def test_cpu_to_cpu_sub_empty(self):
        """CPU pub to sub with '' should receive CPU data."""
