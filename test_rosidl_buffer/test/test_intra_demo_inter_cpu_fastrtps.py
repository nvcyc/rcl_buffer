...
#
# Launch test: 1 pub (demo) → 1 intra-process sub (demo) + 1 inter-process sub (cpu) (FastRTPS)
# Mixed scenario: intra-process sub accepts demo backend (zero-copy path),
# inter-process sub uses default CPU-only (serialization fallback).
...
class TestIntraDemoInterCpuFastRTPS(unittest.TestCase):
    """1 demo pub → 1 demo intra sub + 1 cpu inter sub (mixed, FastRTPS)."""
    ...
    def setUp(self):
        self.node = rclpy.create_node('test_intra_demo_inter_cpu_fastrtps')
        self.intra_count = 0
        self.inter_count = 0
...
class TestIntraDemoInterCpuFastRTPSShutdown(unittest.TestCase):
