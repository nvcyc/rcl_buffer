...
#
# Launch test: 1 pub (demo) → 1 intra-process sub (demo) + 1 inter-process sub (cpu) (Zenoh)
...
class TestIntraDemoInterCpuZenoh(unittest.TestCase):
    """1 demo pub → 1 demo intra sub + 1 cpu inter sub (mixed, Zenoh)."""
    ...
    def setUp(self):
        self.node = rclpy.create_node('test_intra_demo_inter_cpu_zenoh')
        self.intra_count = 0
        self.inter_count = 0
...
class TestIntraDemoInterCpuZenohShutdown(unittest.TestCase):
