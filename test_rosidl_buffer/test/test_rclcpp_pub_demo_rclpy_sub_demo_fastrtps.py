...
#
# Launch test: 1 rclcpp publisher (demo backend) to 1 rclpy subscriber (demo backend). (FastRTPS)
...
# - C typesupport deserializes into a Buffer (is_rosidl_buffer flag on the C struct)
# - convert_to_py wraps the Buffer* into a Python rosidl_buffer.Buffer
...
class TestRclcppPubDemoRclpySubDemoFastRTPS(unittest.TestCase):
    ...
    def setUp(self):
        self.node = rclpy.create_node('test_rclcpp_pub_demo_rclpy_sub_demo_fastrtps')
...
class TestRclcppPubDemoRclpySubDemoFastRTPSShutdown(unittest.TestCase):
