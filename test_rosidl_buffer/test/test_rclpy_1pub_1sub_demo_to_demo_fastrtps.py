...
#
# Launch test: 1 rclpy publisher to 1 rclpy subscriber, demo backend to demo backend. (FastRTPS)
# Validates that the Buffer is_rosidl_buffer path works end-to-end:
# - Python publisher creates a DemoBuffer (backend_type="demo")
# - convert_from_py sets is_rosidl_buffer on the C message
...
# - Python subscriber receives an rosidl_buffer.Buffer
...
class TestRclpyDemoToDemoFastRTPS(unittest.TestCase):
    """Test case for rclpy demo-to-demo image pub/sub via Buffer is_rosidl_buffer flag."""
    ...
    def setUp(self):
        self.node = rclpy.create_node('test_rclpy_demo_to_demo_fastrtps')
...
class TestRclpyDemotoDemoFastRTPSShutdown(unittest.TestCase):
