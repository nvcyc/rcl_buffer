...
#
# Launch test: rclcpp same-process pub/sub, CPU backend (Zenoh)
#
...
# backend works correctly when both endpoints share the same Zenoh session.
...
class TestSameProcessCpuToCpuZenoh(unittest.TestCase):
...
class TestSameProcessCpuToCpuZenohShutdown(unittest.TestCase):
