...
#
# Launch test: demo pub to cpu sub with acceptable_buffer_backends="cpu" (FastRTPS)
# Verifies that explicit "cpu" causes CPU fallback when publisher uses demo backend.
...
def generate_test_description():
    """Demo pub to sub with acceptable_buffer_backends='cpu' (CPU fallback)."""
    publisher_node = Node(
...
class TestDemoCpuSubCpuFastRTPS(unittest.TestCase):
    """Demo to CPU with acceptable_buffer_backends='cpu' (CPU fallback)."""
    ...
    def setUp(self):
        self.node = rclpy.create_node('test_demo_cpu_sub_cpu_fastrtps')
    ...
    def test_demo_to_cpu_sub_cpu(self):
        """Demo pub to sub with 'cpu' should fall back to CPU data."""
...
class TestFastRTPSShutdown(unittest.TestCase):
