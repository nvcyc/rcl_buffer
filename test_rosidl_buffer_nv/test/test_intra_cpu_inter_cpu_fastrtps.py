...
#
# Launch test: 1 pub (CPU) → 1 intra-process sub (CPU) + 1 inter-process sub (CPU) (FastRTPS)
...
class TestIntraCpuInterCpuFastRTPS(unittest.TestCase):
    """1 CPU pub → 1 CPU intra-process sub + 1 CPU inter-process sub (FastRTPS)."""
    ...
    def setUp(self):
        self.node = rclpy.create_node('test_intra_cpu_inter_cpu_fastrtps')
        self.intra_count = 0
        self.inter_count = 0
...
class TestIntraCpuInterCpuFastRTPSShutdown(unittest.TestCase):
