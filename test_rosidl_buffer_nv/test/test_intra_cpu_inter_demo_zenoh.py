...
#
# Launch test: 1 pub (demo) → 1 intra-process sub (cpu) + 1 inter-process sub (demo) (Zenoh)
...
class TestIntraCpuInterDemoZenoh(unittest.TestCase):
    """1 demo pub → 1 cpu-only intra sub + 1 demo-accepting inter sub (mixed, Zenoh)."""
    ...
    def setUp(self):
        self.node = rclpy.create_node('test_intra_cpu_inter_demo_zenoh')
        self.intra_count = 0
        self.inter_count = 0
...
class TestIntraCpuInterDemoZenohShutdown(unittest.TestCase):
