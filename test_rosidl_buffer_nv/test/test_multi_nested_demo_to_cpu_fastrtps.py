...
# CPU-only subscriber over rmw_fastrtps_cpp. Subscriber pins
# `acceptable_buffer_backends = "cpu"` and requires that every nested
# uint8_values Buffer arrive on the CPU backend with identical content.
...
class TestMultiNestedDemoToCpuFastRTPS(unittest.TestCase):
    ...
    def setUp(self):
        self.node = rclpy.create_node('test_multi_nested_demo_to_cpu_fastrtps')
...
class TestMultiNestedDemoToCpuFastRTPSShutdown(unittest.TestCase):
