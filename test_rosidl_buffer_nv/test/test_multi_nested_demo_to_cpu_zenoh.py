...
# CPU-only subscriber over rmw_zenoh_cpp.
#
# Publisher constructs every nested `uint8_values` via DemoBufferImpl; the
# subscriber pins `acceptable_buffer_backends = "cpu"` and requires the
# delivered nested buffers to be CPU-backed. Verifies that nested demo
# buffers are transparently converted to CPU-backed buffers during
# publish/deserialise without losing content.
...
class TestMultiNestedDemoToCpuZenoh(unittest.TestCase):
    ...
    def setUp(self):
        self.node = rclpy.create_node('test_multi_nested_demo_to_cpu_zenoh')
    ...
    def test_demo_to_cpu_nested_buffers(self):
...
class TestMultiNestedDemoToCpuZenohShutdown(unittest.TestCase):
