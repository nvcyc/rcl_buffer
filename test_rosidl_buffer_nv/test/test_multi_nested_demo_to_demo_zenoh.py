...
#
# The publisher constructs every nested `uint8_values` field via
...
def generate_test_description():
    ...
    subscriber_node = Node(
        ...
        parameters=[{
            'topic_name': 'test_multi_nested',
            # Nested uint8_values fields may arrive on "cpu" (RMW downgrades
            # nested Buffers during deserialisation) or "demo" if a future
            # RMW change propagates nested backends. Both count as success
...
class TestMultiNestedDemoToDemoZenoh(unittest.TestCase):
    ...
    def setUp(self):
        self.node = rclpy.create_node('test_multi_nested_demo_to_demo_zenoh')
...
class TestMultiNestedDemoToDemoZenohShutdown(unittest.TestCase):
