...
#
# Launch test: 1 rclpy publisher (CPU) to 1 rclcpp subscriber (CPU). (FastRTPS)
# Validates cross-language interop for CPU-backed uint8[] image data.
...
def generate_test_description():
    """Generate launch description for rclpy CPU pub to rclcpp CPU sub test."""
    publisher_node = Node(
...
class TestRclpyPubCpuRclcppSubCpuFastRTPS(unittest.TestCase):
    """Test case for rclpy CPU publisher to rclcpp CPU subscriber."""
    ...
    def setUp(self):
        self.node = rclpy.create_node('test_rclpy_pub_cpu_rclcpp_sub_cpu_fastrtps')
    ...
    def test_rclpy_pub_cpu_rclcpp_sub_cpu_messages_delivered(self):
        """Test rclpy CPU publisher to rclcpp CPU subscriber."""
...
class TestRclpyPubCpuRclcppSubCpuFastRTPSShutdown(unittest.TestCase):
