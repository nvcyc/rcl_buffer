...
#
# Launch test: TensorList ("list of uint8[]") CPU-backend publisher ->
# CPU-backend subscriber over rmw_fastrtps_cpp. Baseline case: every
# nested `tensor.data` field is built via the standard CPU path and must
# arrive as CPU-backed Buffers.
...
class TestTensorListCpuToCpuFastRTPS(unittest.TestCase):
    ...
    def setUp(self):
        self.node = rclpy.create_node('test_tensor_list_cpu_to_cpu_fastrtps')
...
class TestTensorListCpuToCpuFastRTPSShutdown(unittest.TestCase):
